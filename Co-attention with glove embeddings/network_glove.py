...
class ocr_Encoder(nn.Module):
    def __init__(self, no_layers, dropout, embs_weight):
        super(ocr_Encoder, self).__init__()
        self.embeddings_weight = embs_weight
        self.embedding = torch.nn.Embedding.from_pretrained(torch.from_numpy(self.embeddings_weight).float())
        self.lstm = nn.LSTM(input_size = 100, hidden_size = 256, num_layers = no_layers, bidirectional = True, batch_first=True,
                             dropout =  dropout)
...
class Model(nn.Module):
    ...
    def forward(self, text, entity, feature):
        ...
        D = feature        
        A = (torch.matmul(torch.matmul(C, self.Wa), E.permute(0, 2, 1)) +
            torch.matmul(torch.matmul(D, self.We), E.permute(0, 2, 1)))
